...
func _physics_process(delta: float) -> void:
	...
	# Same wall already faded, skip
	if hit and faded_walls.has(hit):
		return
	...
	if hit:
		_set_wall_transparency(hit.find_children("", "MeshInstance3D")[0], target_alpha)
		faded_walls[hit] = true

func _set_wall_transparency(wall: Node3D, alpha: float) -> void:
